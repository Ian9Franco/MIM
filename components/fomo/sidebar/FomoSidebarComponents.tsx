      ...
      <div className="flex gap-2">
        ...
        {onExplain && (
          <button
            onClick={onExplain}
            disabled={explaining}
            className={`flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold border transition-all disabled:opacity-50 ${isModern ? 'border-black/10 text-black/80 hover:bg-black/5' : 'border-white/10 text-white/80 hover:bg-white/20'}`}
          >
            {explaining ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
            {explaining ? "Explicando…" : "MIM-Bot lote"}
          </button>
        )}
