/**
 * MIMweb — Intelligent Multimodal Project Explainer Service
...
/**
 * Descarga en paralelo de 3 a 5 capturas de pantalla de la galería con timeout de 2.5s
 */
export async function fetchImagesAsInlineData(
  ...
  const fetchPromises = candidates.map(async (imgUrl) => {
    try {
      ...
      // Limitar imágenes a un máximo de 1.5MB por captura para evitar desbordar el payload de Gemini
...
export async function explainModWithGemini(
  ...
  for (const currentModel of modelsToTry) {
    ...
    try {
      ...
      // Si el modelo dio 429 (quota) o 503, loguear e intentar el siguiente modelo en la cascada
      const errText = await response.text();
      console.warn(`[ModExplainer] Modelo ${currentModel} respondió ${response.status}: ${errText.substring(0, 100)}. Probando siguiente modelo de respaldo...`);
    } catch (fetchErr: any) {
      console.warn(`[ModExplainer] Error con modelo ${currentModel}:`, fetchErr.message);
    }
  }

  // Si TODOS los modelos de Google dieron 429 o fallaron, activar el Fallback Heurístico Local de MIM-Bot
