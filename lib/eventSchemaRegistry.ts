...
import { EventSchema, SchemaMigration, ValidationResult } from "./schemas/types";
import { validateAgainstSchema } from "./schemas/Validator";
import { BASE_SCHEMAS } from "./schemas/BaseSchemas";
...
class EventSchemaRegistry {
  ...
  getSchema(eventName: EventName, version: string = "latest"): EventSchema | undefined {
    if (version === "latest") {
      ...
      return this.schemas.get(`${eventName}@${latest}`);
    }
  ...
  validatePayload<T extends EventName>(eventName: T, payload: any, version?: string): ValidationResult {
    ...
    if (!schema) return { valid: false, errors: [{ path: "schema", message: `Schema not found for ${eventName}`, value: eventName }] };
  ...
  private initializeSchemas() {
    BASE_SCHEMAS.forEach(s => this.registerSchema({ ...s, createdAt: new Date().toISOString() } as EventSchema));
  }
