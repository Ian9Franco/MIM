...
- SAGE-08: reemplazada la suite que reproducía lógica del explainer por contratos que importan `modExplainer.ts` y ejecutan también el consumidor HTTP real `/api/fomo/explain`; cubren metadata, imágenes, grounding y fallback con proveedor controlado. La reconciliación final de SAGE-08 queda pendiente de verificación post-merge por Hermione.
...
- REC-03 Phase 2 (en revisión): extraído el ownership de Drafts de `useHomeController` a un sub-hook que conserva estado, persistencia, refresco por evento, entrada a colección y mutaciones detrás del mismo contrato público, con validación defensiva de datos y regresiones dedicadas.
