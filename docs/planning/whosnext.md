...
- [ ] **SAGE-01 — Procedencia y generalización del corpus (Capa A en código; no cerrado).** Schema Zod con `origin`/`license`/`anonymized`/`split`/`duplicateOf`; 125 casos históricos quedan `origin: unknown` `split: train`; detector de duplicados (`--audit`); desglose por loader; `split: stress` (10 sintéticos, no entran al gate SAGE-03); `split: holdout` vacío hasta logs reales no usados para tunear reglas. El 100% F1 de train no se presenta como generalización.
