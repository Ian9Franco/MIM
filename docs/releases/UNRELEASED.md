...
- CI-HUB-01: corregida la raíz de Turbopack del Hub para el workspace monorepo; el build remoto en `main` fallaba al resolver `next/package.json` desde `web/app` con `turbopack.root` limitado a `web/`. La verificación final depende del CI remoto de este PR.
