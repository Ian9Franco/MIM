...
function toMod(f: Record<string, unknown>): ModHit {
  ...
  const projectType = String(f.project_type || f.content_type || "mod");
  let title = String(f.name || "Proyecto"), author = "Comunidad";
  if (title.includes(" ::: ")) [title, author] = title.split(" ::: ");
  const source = String(f.platform || f.source || "modrinth");
  return {
    projectId,
    title,
    author,
    ...
    iconUrl: ((f.icon_url || f.iconUrl) ?? undefined) as string | undefined,
    categories: Array.isArray(f.categories) ? (f.categories as string[]) : [],
    _source: source,
    url: (f.url as string) || (source === "curseforge" ? `https://www.curseforge.com/projects/${projectId}` : `https://modrinth.com/${projectType}/${projectId}`)
  };
}
...
export function CollectionsTab(p: Props) {
  ...
  const visibleMods = useMemo(() => (p.activeCollectionMods as Array<ModHit & { versions?: string[]; game_versions?: string[]; loaders?: string[]; loader?: string; itemId?: string; id?: string }>).filter(m => {
    const type = String(m.projectType || "mod").toLowerCase();
      ...
      {!p.activeCollection ? <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex min-h-0 flex-1 flex-col overflow-y-auto pb-28 scrollbar-none">
        ...
        <AnimatePresence mode="wait">
          {view === "editorial" && <motion.section key="editorial" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-6">
            ...
            {!!p.curseForgeFeatured.length && <div><Title eyebrow="Otra fuente" title="CurseForge Picks" /><div className="mt-2 space-y-2">{p.curseForgeFeatured.map(c => <Row key={c.id} c={c} open={(item) => { p.handleEnterCollection(item); }} />)}</div></div>}
            {!!archive.length && <div className="rounded-2xl border border-border bg-surface/55 p-2">
              ...
              <AnimatePresence>{archiveOpen && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden"><div className="space-y-2 border-t border-border px-1 pt-3"><SearchBox value={archiveQuery} setValue={setArchiveQuery} placeholder="Buscar en el archivo..." /><div className="mim-chip-scroll">{["Todos", ...years.map(y => y)].length > 0 && <Chip active={archiveYear === "Todos"} click={() => { setArchiveYear("Todos"); }}>Todos</Chip>}{years.map(y => <Chip key={y} active={archiveYear === y} click={() => { setArchiveYear(y); }}>{y}</Chip>)}</div>{visibleArchive.map(c => <Row key={c.id} c={c} open={(item) => { p.handleEnterCollection(item); }} compact />)}{!visibleArchive.length && <p className="py-6 text-center text-[9px] text-white/35">No encontramos selecciones.</p>}</div></motion.div>}</AnimatePresence>
            </div>}
          </motion.section>}
