...
async function checkVirusTotalHash(sha256: string): Promise<SecurityScanResult["virusTotal"]> {
  ...
  try {
    ...
    if (res.status === 404) return { maliciousCount: 0, totalEngineCount: 0 };
...
export async function scanSecurity(filePath: string): Promise<SecurityScanResult> {
  ...
  if (isTrusted) return buildResult(0, [{ category: "manifest_anomaly", severity: "info", description: `🛡️ Mod Confiable (${meta.modId || "Verificado"})`, details: ["Popular y verificado por la comunidad."], scoreImpact: 0 }], sha1, scannedAt, sha256, null);
...
function buildResult(score: number, findings: SecurityFinding[], sha1: string, scannedAt: string, sha256: string, vt: any): SecurityScanResult {
...
/**
 * Performs security scans on multiple files in parallel.
 */
export async function scanSecurityBatch(filePaths: string[]): Promise<Record<string, SecurityScanResult>> {
  const results: Record<string, SecurityScanResult> = {};
  await Promise.all(filePaths.map(async (path) => {
    try { results[path] = await scanSecurity(path); } catch {}
  }));
  return results;
}
