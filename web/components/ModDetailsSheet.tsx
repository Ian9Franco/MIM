...
function renderBodyText(body: string) {
  ...
  return <div className="mim-rich-description" dangerouslySetInnerHTML={{ __html: richDescriptionHtml(body) }} />;
}

async function translateDescription(projectId: string, body: string) {
  if (descriptionTranslationCache[projectId]) return descriptionTranslationCache[projectId];
  ...
  return (
    ...
    >
      <motion.div
        ...
        transition={{ type: "spring", stiffness: 360, damping: 26, mass: 0.9, bounce: 0.22 }}
        ...
        <div className="flex flex-col gap-5 p-6 flex-1 min-h-0">
          ...
          <div className="flex gap-4 shrink-0">
            ...
            <div className="flex flex-col gap-1.5 absolute right-5 top-5">
              ...
              <button
                onClick={closeWithSound}
                className="bg-white/5 hover:bg-white/10 rounded-full p-1.5 text-white/60 active:scale-95"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
