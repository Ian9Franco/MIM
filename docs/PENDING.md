...
| :--- | :--- | :--- |
| **Persistencia durable de Snapshots (SRV-3 core):** `FileSnapshotStore`, detección (`loadPendingServerOperations`), `pendingOperations` en inspect y store en deploy. Pendiente: validación en hosting real y recovery UI avanzada. | `SRV-3` | 📘 [server-manager.md](./architecture/server-manager.md)<br/>📌 [ROADMAP.md (Sección 9)](./planning/ROADMAP.md#9-server-manager--sincronización-remota-issue-58--en-progreso) |
