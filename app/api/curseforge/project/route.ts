      ...
      try {
        if (modData.data && modData.data.slug) {
          ...
          if (mrRes.ok) {
            ...
            const queryName = encodeURIComponent(modData.data.name);
            const mrSearch = await fetch(`https://api.modrinth.com/v2/search?query=${queryName}&limit=5`);
            if (mrSearch.ok) {
