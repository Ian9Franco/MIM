...
class MimEventBus {
  ...
  private processEvent<T extends EventName>(event: T, payload: EventPayload<T>): void {
    ...
    if (handlers) {
      handlers.forEach((handler) => {
        try {
          ...
          console.error(`[MIM Bus] Error en handler para ${String(event)}:`, error);
        }
      });
    }
    ...
    if (typeof window !== "undefined") {
      const domEvent = new CustomEvent(`mim:${String(event)}`, { 
        detail: payload,
        bubbles: true 
      });
    ...
    if (process.env.NODE_ENV === "development") {
      console.log(`%c[MIM Bus] %c${String(event)}`, "color: #818cf8; font-weight: bold", "color: #fff", payload);
    }
  }
