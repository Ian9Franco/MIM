...
interface CommunityEditProfileModalProps {
  ...
  handleSaveProfile: (e: React.FormEvent, finalAvatar?: string | null) => Promise<void>;
...
export function CommunityEditProfileModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await handleSaveProfile(e, finalAvatar);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
      <form 
        ...
        className={`border rounded-2xl p-6 w-full max-w-sm space-y-4 shadow-2xl relative ${isModern ? 'bg-card text-foreground border-border shadow-[0_20px_60px_rgba(13,39,80,0.12)]' : 'bg-[#121214] border-white/10'}`}
      >
