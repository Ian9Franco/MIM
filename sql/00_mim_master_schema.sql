...
-- =========================================================================================
-- 5. COMMUNITY SOCIAL LAYER (Profile follows & reactions)
-- =========================================================================================
...
CREATE POLICY "Lectura pública de perfiles seguidos" ON public.followed_profiles FOR SELECT USING (true);
CREATE POLICY "Seguir desde perfil propio" ON public.followed_profiles FOR INSERT WITH CHECK (auth.uid() = follower_id);
CREATE POLICY "Dejar de seguir desde perfil propio" ON public.followed_profiles FOR DELETE USING (auth.uid() = follower_id);
...
CREATE POLICY "Lectura pública de reacciones" ON public.community_reactions FOR SELECT USING (true);
CREATE POLICY "Reaccionar desde perfil propio" ON public.community_reactions FOR INSERT WITH CHECK (auth.uid() = profile_id);
CREATE POLICY "Quitar reacción propia" ON public.community_reactions FOR DELETE USING (auth.uid() = profile_id);
