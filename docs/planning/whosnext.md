...
- [x] **BOT-07 — Cola para análisis por lotes (parcial, working tree 2026-09-12).** `runAnalysisQueue` (concurrencia por proveedor, retry de cuota, cancelación). Eval live `eval:mimbot` usa la cola. `POST /api/fomo/explain-batch` + botón **MIM-Bot lote** en Descubrir (máx. 12). Falta merge a main y gate live con secrets confirmados (SAGE-05b).
